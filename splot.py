...
def main():
    ...
    ## if available load bad data points and remove them from the list ##
    data = getdat.rmbaddat(eventname,datfiles,data)
...
#    print parmfree
    ...
    parmfree,parmfix,chi2s,dof,fsfb = sfitpy.downhill(sfitpy.lnprob_func,namefree,parmfree,namefix,parmfix,data,[alpha,delta,oblats,oblongs,obgammas],parmref,isspitz,use_color=use_color,I_minus_L=I_minus_L)
    ### MCMC to minimize the chisq ###
#    parmfree,parmfix,chi2s,dof,fsfb = sfitpy.mcmc(sfitpy.lnprob_func,eventname,namefree,parmfree,namefix,parmfix,data,[alpha,delta,oblats,oblongs,obgammas],parmref,isspitz,use_color=use_color,I_minus_L=I_minus_L)
#### use getchi2 instead of lnprob_func: TBD ###
    ### Downhill simplex method ###
#    parmfree,parmfix,chi2s,dof,fsfb = sfitpy.downhill(sfitpy.getchi2,namefree,parmfree,namefix,parmfix,data,[alpha,delta,oblats,oblongs,obgammas],parmref,isspitz,use_color=use_color,I_minus_L=I_minus_L)
#    ### MCMC to minimize the chisq ###
#    parmfree,parmfix,chi2s,dof,fsfb = sfitpy.mcmc(sfitpy.getchi2,eventname,namefree,parmfree,namefix,parmfix,data,[alpha,delta,oblats,oblongs,obgammas],parmref,isspitz,use_color=use_color,I_minus_L=I_minus_L)
#### Find the model according to the given parameters ###
    ...
    npts = (7300-7100)*10000./te
    npts = min([npts,100000])
    tmod = np.linspace(7100,7300,int(npts))
    for iob in range(len(data)):
        ## find model ##
...
#    mmod = np.array(mmod)
    ...
    print ['%.2f'%np.sqrt(chi2s[i+1]*1.0/dof[i]) for i in range(len(dof))]
### start plotting ###
...
#    colorstr = ['k','r','g','m']
    ...
    for iob in range(len(data)):
        ...
        ax.errorbar(date,imag,yerr=imerr,marker='o',linestyle='none',color=colorstr[iob],markeredgecolor=colorstr[iob],markerfacecolor='none',label='nob=%d'%(iob+1))
        ## plot model ##
