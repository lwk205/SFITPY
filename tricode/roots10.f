...
! _1_                    CMPLX_ROOTS_GEN                            !
!-------------------------------------------------------------------!
c      subroutine cmplx_roots_gen(roots, poly, degree, 
c     *            polish_roots_after, use_roots_as_starting_points)
c      ! This subroutine finds roots of a complex polynomial. 
c      ! It is general, however less fast or robust than cmplx_roots_5
c      ! which contains failsafe checks in the polishing stage, but is
c      ! designed only for 5th order polynomials.
c      ! It uses a new dynamic root finding algorithm (see the Paper).
c      !
c      ! It can use Laguerre's method (subroutine cmplx_laguerre)
c      ! or Laguerre->SG->Newton method (subroutine 
c      ! cmplx_laguerre2newton - this is default choice) to find 
c      ! roots. It divides polynomial one by one by found roots. At the 
c      ! end it finds last root from Viete's formula for quadratic 
c      ! equation. Finally, it polishes all found roots using a full
c      ! polynomial and Newton's or Laguerre's method (default is
c      ! Laguerre's - subroutine cmplx_laguerre). 
c      ! You can change default choices by commenting out and uncommenting
c      ! certain lines in the code below.
c      !
c      ! Note:
c      ! - we solve for the last root with Viete's formula rather 
c      !   than doing full Laguerre step (which is time consuming
c      !   and unnecessary)
c      ! - we do not introduce any preference to real roots
c      ! - in Laguerre implementation we omit unneccesarry calculation of
c      !   absolute values of denominator
c      ! - we do not sort roots. If you need to sort 
c      !   roots - we have provided sorting subroutine called:
c      !   sort_n_points_by_separation, which sorts points from most 
c      !   isolated to most close. Algorithm in this routine can be 
c      !   easily used for number of points different than 10.
c      !
c      implicit none
c      ! roots  - array which will hold all roots that had been found. 
c      !          If the flag 'use_roots_as_starting_points' is set to 
c      !          .true., then instead of point (0,0) we use value from
c      !          this array as starting point for cmplx_laguerre
c      ! poly -   is an array of polynomial cooefs, length = degree+1, 
c      !          poly(1) is a constant term:
c      !               1              2             3
c      !          poly(1) x^0 + poly(2) x^1 + poly(3) x^2 + ...
c      ! degree - degree of the polynomial and size of 'roots' array
c      ! polish_roots_after - after all roots have been found by dividing
c      !          original polynomial by each root found,
c      !          you can opt in to polish all roots using full  
c      !          polynomial
c      ! use_roots_as_starting_points - usually we start Laguerre's 
c      !          method from point (0,0), but you can decide to use the 
c      !          values of 'roots' array as starting point for each new
c      !          root that is searched for. This is useful if you have
c      !          very rough idea where some of the roots can be. 
c      !
c      integer degree ! intent(in)
c      complex*16 poly(degree+1) ! intent(in), coeffs of the polynomial
c      complex*16 roots(degree) ! intent(inout)
c      logical polish_roots_after, use_roots_as_starting_points ! intent(in)
c
c      complex*16 poly2(degree+1)
c      complex*16 zero
c      parameter(zero=(0d0,0d0))
c      integer i, n, iter
c      logical success
c      complex*16 coef, prev
c
c
c      do i=1,degree+1
c        poly2(i)=poly(i)
...
c      if(.not.use_roots_as_starting_points) then
c        do i=1,degree
c          roots(i)=zero
c        enddo
c      endif
c
c      ! skip small degree polynomials from doing Laguerre's method
c      if(degree.le.1)then
c        if(degree.eq.1) roots(1)=-poly(2)/poly(1)
c        return
c      endif
c
c
c      do n=degree, 3, -1
c
c        ! find root with Laguerre's method
c        !call cmplx_laguerre(poly2, n, roots(n), iter, success) 
c        ! or
c        ! find root with (Laguerre's method -> SG method -> Newton's method)
c        call cmplx_laguerre2newton(poly2, n, roots(n), 
c     *                                     iter, success, 2)
...
c        endif
c
c        ! divide the polynomial by this root
c        coef=poly2(n+1)
c        do i=n,1,-1
c          prev=poly2(i)
c          poly2(i)=coef
c          coef=prev+roots(n)*coef
c        enddo
c        ! coef now holds a remainder - should be close to 0
c
c      enddo
c
c      ! find all but last root with Laguerre's method
c      !call cmplx_laguerre(poly2, 2, roots(2), iter, success)
c      ! or
c      call cmplx_laguerre2newton(poly2, 2, roots(2), 
c     *                                   iter, success, 2)
c      if(.not.success) then
c        call solve_quadratic_eq(roots(2),roots(1),poly2)
c      else
c        ! calculate last root from Viete's formula
c        roots(1)=-(roots(2)+poly2(2)/poly2(3))
c      endif
c
c
c
c      if(polish_roots_after)then
c        do n=1, degree ! ! polish roots one-by-one with a  full polynomial 
c          call cmplx_laguerre(poly, degree, roots(n), iter, success) 
c          ! or
c          !call cmplx_newton_spec(poly, degree, roots(n), iter, success)
c        enddo
c      endif  
c
c     
...
!-------------------------------------------------------------------!
     ...
     * (roots, first_n_minus_2_roots_order_changed, poly, polish_only)
      ...
      !
      integer degree
      parameter(degree=10) ! specifying this speeds up optimization a little
...
!-------------------------------------------------------------------!
      ...
      implicit none
      integer n
      parameter(n=10) !  works for different n as well, but is faster for n as constant (optimization)
...
!-------------------------------------------------------------------!
      ...
      implicit none
      integer n
      parameter(n=10) ! will work for other n too, but it is faster with n as constant
...
!-------------------------------------------------------------------!
      ...
      real*8 faq ! jump length
      real*8 FRAC_ERR
! fractional error for kind=8 (see. Adams 1967 Eqs 9 and 10)
      parameter(FRAC_ERR = 2.0d-15) 
...
!-------------------------------------------------------------------!
      ...
      real*8 third
      parameter(zero=(0d0,0d0))
      parameter(zeta =(-0.5d0, 0.8660254037844386d0))
      parameter(zeta2=(-0.5d0,-0.8660254037844386d0))
