...
#    namefix = np.hstack(['pien','piee',nameflx.flatten()])
#    namefix = nameflx.flatten()
    ### TBD: how to have all parameters free??? ###
...
def getparmfree(parmfit,parmflx,namefree,namefix):
    parmflx = parmflx.flatten()
    parmall = np.hstack([parmfit,parmflx])
    nameall = np.hstack([namefree,namefix])
    parmfree= [parmall[nameall==i] for i in namefree]
    parmfix = [parmall[nameall==i] for i in namefix]
    parmfree= np.array(parmfree).flatten()
    parmfix = np.array(parmfix).flatten()
    return parmfree,parmfix
...
#    print chi2_obs,dof
    ...
    pos,lnprob,rstate = sampler.run_mcmc(pos,50)  ## burn in
    sampler.reset()
    ...
    return parmbest,parmfix,np.hstack([chi2min,chi2_obs]),dof,fsfb
...
#            continue
    ...
    for i in range(nlc):
        data_sub = lcs_sub[i]
        iparmfree_sub,parmfix_sub,chi2_sub,dof_sub,fsfb_sub = downhill(lnprob_func,namefree_sub,parmfree_sub,namefix_sub,parmfix_sub,data_sub,eventinfo_sub,parmref,isspitz_sub,use_color=use_color_sub,I_minus_L=I_minus_L)
        if chi2_sub[0] > 10:
            chi2 = np.ones(1+nobs)
        else:
            data = lcs[i]
...
##                plt.show()
#                np.savetxt('output/models/fit-ilc=%d-iob=%d.dat'%((i+1),(iob+1)),np.vstack([lcs_full[i][iob],ifmod]),fmt='%f',header='fs=%f, fb=%f'%(iparmflx[iob][0],iparmflx[iob][1]))
        bestfits.append(np.hstack([q,s,thetas[i],chi2.reshape(-1),chi2_sub.reshape(-1),iparmfree_sub,parmfix_sub]))
