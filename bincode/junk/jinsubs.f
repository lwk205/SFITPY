...
c     Packaging for Jin's code. To be expanded to include limb-darkening.
      ...
      integer n
      parameter (n=10) !number of rings used to approximate the limb-darkening
...
c      magbps = 0d0
     ...
     +     rhos, errorflag)     !errorflag = 0 means no error.
...
c      err = .false.
      ...
      call getim3(xim,yim,amp,nsolc,b,q,xsi0,eta0,magpt)
      iii = 0
      ...
      xsi = xsi0 + rhostar
      eta = eta0
      call getim3(xim,yim,amp,nsol0,b,q,xsi,eta,mag)
      ncrs = 0
      check = .false.
      theta = 0.d0
      mstep = 0
      do 1 i=1,nstep
c$$$         thetaadd = 2.d0*PI/5.d2
c$$$         if(mag.ge.10.d0)thetaadd = 2.d0*PI/(5.d1*mag)
c$$$         if(mag.ge.800.d0)thetaadd = 2.d0*PI/4.d4
         ...
         xsi = dreal(z(mstep))
         eta = dimag(z(mstep))
         call getim3(xim,yim,amp,nsol,b,q,xsi,eta,mag)
         k = 0
c         if(nsol.ne.5)then
...
c         endif
 ...
 2       continue
     ...
     *      sols,b,q,zcen,rhostar,nsol0,mcrs,nstep)
            poscrs(ncrs) = mcrs
         endif
         nsol0 = sols(mstep)
         if(check)go to 333
 1    continue
      write(6,*)'not enough count'
      call getmagf(magfs,magpt,xsi0,eta0,rhostar,b,q)
      return
 333  continue
      if(mstep.gt.nstep)then
         write(6,*)'too much count'
         call getmagf(magfs,magpt,xsi0,eta0,rhostar,b,q)
         return
      endif
      check = .false.
     ...
     *                 .or.ncrs.gt.10)then
         write(6,*)' Wrong crossing points',ncrs
c         stop
          err = -1 !Wrong crossing points detected, use wei_code
          return
      endif
      ...
      do 3 i=1,mstep-1
         ...
         do 25 j=1,ncrs
            if(poscrs(j).eq.ip)then
               ...
               in = in + 1
               if(in.gt.ncrs)then
                write(6,*)'crossing points counting error in getmagff'
                stop
               endif
               posend(in,1) = ip
               do 10 k=1,sols(ip)
      ...
      if(ncrs.eq.0)then
         ...
         do 21 i=1,sols(posst)
            ...
            do 22 j=1,sols(posst)
               ...
               if(dif.lt.difm)then
                  write(6,*)'wrong image topology A'
c                  magfs = magpt*1.d2
                  magfs = magpt
c                  err = .true.
                  err=1.
 ...
 33      continue
      ...
      if(difm.ge.1.d0)then
         write(6,*)'cannot find the connected patch in getmagff'
c          magfs = 1.d2*magpt
          magfs = magpt
c          err = .true.
          err = 2.
...
*
     ...
     *           sols,b,q,zcen,rhostar,nsol0,mcrs,nstep)
      ...
      call getim3(xi,yi,am,nsol,b,q,xsi,eta,mag)
...
***  getim0
*
      subroutine getim0(xi,yi,am,nsol,b,q,xsi,eta,magtot)
...
***  getim2
*
      subroutine getim2(xi,yi,am,nsol,b,q,xsi,eta,magtot)
CU    uses getim,(or getim0)
      ...
      knt = 0
 1    continue
      call getim0(xi,yi,am,nsol,b,q,xsi,eta,magtot)
...
***   getim3
*
      subroutine getim3(xii,yii,amm,nsol,b,q,xsi,eta,magtot)
CU    uses getim2
      double precision b,q
      double precision xsi,eta
      double precision xi(5),yi(5),am(5)
...
c      double precision xik,yik,amk
      ...
      call getim2(xi,yi,am,nsol,b,q,xsi,eta,magtot)
...
*** getmagf
*
      subroutine getmagf(magfs,magpt,xsi0,eta0,rhostar,b,q)
